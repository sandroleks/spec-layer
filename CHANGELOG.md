...
- **Empty writing sections are drawn as placeholders instead of left out.**
  ...
  Text typed into a placeholder is recorded as written by a person, and
  stays recorded after later Updates. The brief's and the Component Context
  v5 artifact's `guidelines` block lists those fields under `authored`, after
  `origin: generated`, or reads `origin: authored` when a person wrote every
  field in it; a field built partly from AI text, such as Interactions with a
  typed Keyboard row beside AI-written Pointer bullets, is not listed. The
  Markdown marks a section a person wrote "Written on the Figma canvas, not
  generated by AI." in place of the AI line. A rebuild never asks AI to
  rewrite a key a person wrote, Keyboard included. The `spec-layer` skill
  guide describes both markers. Output for docs without typed-in text is
