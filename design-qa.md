...
- Source visual truth:
  ...
  (860 × 606), `codex-clipboard-9aa4c895-c46f-4cf8-bede-fd5d02e2dfe3.png`
  (860 × 732), and the original compact-control reference
  `codex-clipboard-132c340e-b6ef-4144-8e55-62551233f811.png`
  (936 × 936).
- Browser-rendered implementation:
  `docs/plugin-ui-vnext/audits/2026-07-29-section-list-visual-refinement/01-specifications-inline-controls.png`
  and `03-usage-flat-rows.png`, both 480 × 680.
- CSS viewport: 480 × 680 at 1× density. Focused comparisons normalize each
  implementation crop to the source crop height; the full browser captures
  remain unscaled.
- State: dark Component screen with Usage or Specifications expanded. A light
  Specifications state was checked separately.

## Findings

- [P1] Rounded row outlines broke the flat list rhythm.
  Evidence: the reported Usage capture shows curved separator ends around
  every row. The implementation had placed `border-radius` and a three-pixel
  spread shadow on rows that already owned separators.
  Fix: remove row radius/shadow and transition only the background color.
- [P1] Checkbox focus looked like a persistent selected row.
  Evidence: the reported Measurements row remained filled with accent blue.
  Fix: remove the row-level `:focus-within` fill and keep focus feedback on the
  actual checkbox/radio control.
- [P2] Nested controls were oversized and visually disconnected from their
  parent section.
  Evidence: Anatomy rendered as a segmented control and Measurements as large
  chips, unlike the original compact inline radio/checkbox treatment.
  Fix: restore `Anatomy as` and `Measure` inline labels, a quiet left guide,
  compact native-choice semantics, and single-line spacing.

## Required fidelity surfaces

- Typography: existing family and optical weights are unchanged; nested labels
  use the muted small-control scale from the surrounding plugin.
- Spacing/layout: rows are flat 36px list items; nested choices fit in one line
  with a 25px parent indent and 9px guide padding.
- Colors/tokens: hover uses `--sl-color-surface-subdued`; focus uses the
  existing checkbox/radio focus token rather than a row-wide accent fill.
- Image/asset quality: no image assets were required or replaced. Existing
  icon assets and checkbox glyphs are reused.
- Copy/content: `Anatomy as`, `Measure`, and all option labels match the
  original UI.

## Comparison history

1. Initial pass: blocked by P1 rounded row outlines, P1 persistent accent row
   fill, and P2 oversized nested controls.
2. Fix pass: flattened row geometry, localized focus, and replaced nested
   segmented/chip controls with compact inline choices.
3. Post-fix evidence:
   `04-usage-source-vs-refined.png` and
   `05-specifications-source-vs-refined.png`. The earlier P1/P2 findings are
   no longer present.

## Interaction and browser verification

- Anatomy radio selection and Measurement checkbox selection were exercised in
  the browser.
- Focused checkbox state leaves the row background transparent with no shadow
  or radius.
- Dark and light bodies are exactly 480 × 680 with no body or nested-control
  horizontal overflow.
- Browser warning/error log: empty.
- Focused region comparisons were required because the row geometry and nested
  controls were too small to judge accurately in the full-screen captures.
