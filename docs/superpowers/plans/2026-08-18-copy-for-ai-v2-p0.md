# Copy for AI v2 — P0 (correctness) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the two Copy for AI briefs correct — move contrast from components to foundation colours, give every part one identity, stop `unbound` contradicting `tokens`, and carry the generated foundation prose that is currently dropped.
...
**Scope:** P0 only. P1 (the breaking format change: `tokens.used`, condition-based bindings, `api` split, typography, `validation`, generated boundary) gets its own plan after P0 is verified in Figma.
...
- `packages/extractor/src/brief.ts` — `source` split, `mode` on values, `unbound` reconciliation, foundation prose and contrast.
...
Promote the private `concrete` to an export, because `colorContrast.ts` needs it in Task 5:
...
### Task 3: Classify foundation colour variables by role
...
### Task 4: Report which WCAG bars a ratio clears
...
### Task 5: Measure within-collection pairs per mode
...
**Interfaces:**
- Consumes: `colorRole` and `barsCleared` (Tasks 3-4); `concreteColor`, `contrastRatio`, `blend` from `contrast.ts` (Task 2); `FoundationSpec`, `FoundationVariable` from `foundation.ts`.
- Produces:
...
### Task 6: Add the foundation contrast toggle
...
### Task 7: Render the contrast matrix on the foundation frame
...
**Interfaces:**
- Consumes: `ColorContrastReport`, `ContrastMatrix` (Task 5); `FoundationConfig.includeContrast` (Task 6).
- Produces:
...
### Task 8: Give every part one identity
...
### Task 9: Stop `unbound` contradicting `tokens`
...
**Interfaces:**
- Consumes: `Gap.path` and `TokenRule.path` (Task 8).
...
```ts
/** Re-cut on 2026-08-18 by P0 Task 9: gap issue strings became stable ids, which
...
### Task 10: State which mode a resolved value came from

The spec lists mode-awareness in P0 while also calling P0 non-breaking. Turning `resolved` into a per-mode map is a format break, so P0 adds an additive `mode:` field naming the mode the existing value was read at, and the full per-mode map lands with `tokens.used` in P1.
...
### Task 12: Carry the generated foundation prose and contrast in the foundation brief

This is the task that fixes step 1 of the intended workflow. Today "copy the foundation so the agent sets a vocabulary" ships a bare token table with none of the meaning the plugin already generated onto the frame.
...
**Interfaces:**
- Consumes: `ColorContrastReport` (Task 5); `FoundationDocLink.groupDescriptions` (existing).
- Produces:
...
export function foundationBrief(f: FoundationSpec, opts: FoundationBriefOptions): YamlValue
```
...
it('carries contrast failures and counts but never the full matrix', () => {
  const brief = foundationBrief(oneCollection(), {
    ...
    contrast: {
      ...
      matrices: [{ collection: 'Primitives', mode: 'Light', foregrounds: ['a'],
                   backgrounds: ['b'], cells: [[{ ratio: 2.23, clears: [] }]] }],
      failures: [{
        collection: 'Primitives', mode: 'Light',
...
it('omits contrast when none was computed', () => {
  const brief = foundationBrief(oneCollection(), { generatedAt: 'T' }) as Record<string, any>;
  expect('contrast' in brief).toBe(false);
});
```

Add the fixture helper `oneCollection()` next to the existing foundation brief tests, returning a `FoundationSpec` with one single-mode collection holding one COLOR variable.
...
Change only the signature and add the two blocks. Keep the existing `collections` and `text_styles` mapping bodies exactly as they are.
...
export interface FoundationBriefOptions {
   ...
   * and keyed collection then folder. Nested rather than flat because two
   * collections can each hold a folder of the same name, which a flat map would
   ...
   * doc at all. Never generated here.
   */
  groupDescriptions?: Record<string, Record<string, string>>;
  contrast?: ColorContrastReport;
}
...
export function foundationBrief(
  ...
  return {
    ...
    source: {
      file_key: foundation.fileKey && foundation.fileKey !== 'unknown'
        ? foundation.fileKey : undefined,
    },
    collections: /* unchanged */ undefined as never,
    text_styles: /* unchanged */ undefined as never,
    // Failures and counts only. The matrix belongs on the frame, where a grid is
    // readable; in a brief it is hundreds of lines of mostly-passing cells.
    // `measured` and `unclassified` keep the distinction a bare list destroys: a
    // short failure list means "checked and mostly clean" only when measured is
    // non-zero.
...
Replace the two `undefined as never` placeholders with the existing mapping expressions, copied verbatim from the current implementation.

- [ ] **Step 4: Read descriptions and compute contrast at the copy site**

In `copyFoundationBrief`, gather stored descriptions from the foundation doc links before building the YAML. Find how links are enumerated:

```bash
grep -rn "parseDocLink\|isFoundationLink" packages/plugin/src/ui/actions.ts packages/plugin/src/main.ts | head
```

Merge each foundation link's `groupDescriptions` under its scope's `collectionName` (a `textStyles`-target scope has none, so skip it), then:
...
Add a test to `packages/plugin/test/actions.test.ts` asserting the copied YAML contains `group_descriptions` when a foundation doc with descriptions exists, and omits it when none do.
...
git commit -m "feat(extractor,plugin): carry foundation prose and contrast in the brief" -m "The foundation frame renders AI-written group descriptions, but foundationBrief emitted only collections and text_styles. Step 1 of the intended workflow, copy the foundation so the agent sets a vocabulary, handed over a bare token table with none of the meaning the plugin had already generated." -m "Descriptions are read from the stored doc links, never regenerated, and nested under their collection because two collections can share a folder name. Contrast carries failures and counts only: the matrix belongs on the frame." -m "Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>"
```

---

### Task 13: Verify in Figma
...
**Files:**
- Create: `docs/manual-tests/2026-08-18-copy-for-ai-v2-p0.md`
...
Record every result in `docs/manual-tests/2026-08-18-copy-for-ai-v2-p0.md`. A line that cannot be checked is recorded as "not checked", never as a pass.
...
- [ ] Existing component docs show "Update available" once (expected: Task 9 changed gap issue ids) and settle after a single Update rather than reporting drift again.
...
- [ ] Copy a component brief: no `contrast:` block; `source.file_key` and `source.file_name` hold real values; `unbound` no longer names a path and property that `tokens` binds; each resolved binding carries `mode`.
...
```bash
git add docs/manual-tests/2026-08-18-copy-for-ai-v2-p0.md
git commit -m "docs: record the Copy for AI v2 P0 manual Figma pass" -m "Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>"
```
...
Not in this plan, and not started until P0 passes Task 13: `tokens.used` plus condition-based `tokens.bindings`, `api` split into variants/states/booleans, typography resolved structurally, the `validation` block, `guidelines.origin` on component briefs with the generated-content boundary scan, the payload size assertion, and the golden-file test.

Three P0 decisions P1 must honour:

- `Gap.issue` values are stable ids from `GapIssue`, not prose, and every gap carries a `property` and optionally a `value`.
- Resolved bindings carry `mode`. P1 replaces that single field with a per-mode map inside `tokens.used`, which is the breaking half and the reason it is not here.
- `path` is the join key. `part` remains only for display, and P1's `bindings` must key on `path`.
