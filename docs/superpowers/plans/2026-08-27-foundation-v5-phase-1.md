...
- **v4 alias resolution is stated, not improvised.** In order: (1) match on `(collection, path)` when the v4 alias carries a collection; (2) otherwise match on path alone, and accept **only** if exactly one token matches; (3) if two or more match, emit `kind: alias` with `resolved.status: 'unresolved'`, `reason: 'ambiguous_target'`, and an `AMBIGUOUS_ALIAS_TARGET` diagnostic naming every candidate in `details`. Never take the first match.

  **Step 1 cannot currently reach `resolved`, and that is a property of v4, not a bug.** `valueOf` (`brief.ts:100-109`) emits `collection` only on EXTERNAL aliases, and an external alias by definition has no `resolved` value — so a v4 alias that carries a collection always lacks the value step 1 would need to resolve to. Step 1's matching still runs and still narrows candidates for `target_id`, `target_collection_id` and the step-3 ambiguity check; only its success terminal is unreachable. Keep the code (it becomes reachable the moment backlog item A4 qualifies internal aliases too) and say so in a comment, rather than deleting a branch that will be needed.
...
- **`completeness` is read from v4's `unavailable` list** (`foundation.ts:137`), with `unavailable_sources` sorted by code unit.
  ...
  **Caveat, and it is a real gap:** `FoundationSpec.unavailable` exists on the internal spec but `foundationBrief` never serializes it (verified against `brief.ts:209-274`), so no v4 YAML document in existence carries the field. The normalizer's input type must say so plainly in a comment — an input contract that quietly implies a field real documents have is the kind of fiction this artifact exists to prevent. Emit `completeness: 'complete'` when the field is absent, since that is the only thing a document without it can be read as, and **defer wiring `unavailable` into `foundationBrief` to plan 2**, where extraction changes belong. Do not widen this task into the live v4 emitter.
