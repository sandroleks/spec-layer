...
export async function generateDocs() {
  ...
  for (const page of pages) {
    ...
    // Versioned package names can look like email addresses to Cloudflare.
    content = content.replace(/<code\b[^>]*>[\s\S]*?<\/code>/g, code => `<!--email_off-->${code}<!--/email_off-->`);
