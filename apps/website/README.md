...
The static marketing and documentation website for Spec Layer. This directory is the production source, and the only path to https://spec-layer.com. It also serves the two published JSON schemas from `public/schemas/`.
...
- `npm run build` generates pages and SEO metadata in preview mode, then copies public assets into `dist/`. It requires the shared brand package in this monorepo; no new runtime dependencies are needed.
- `npm run check` builds and checks local routes, links, schemas, examples, SEO metadata, redirects, social image dimensions, indexing rules, and shared brand/asset parity.
...
Support and all four policies are integrated at their existing clean URLs, using source verified against the public site on 6 September 2026. `check-pages.mjs` checks that the generator reproduces each authored body verbatim. Checkout URLs are the existing monthly and annual Lemon Squeezy variants. The developer guide matches the published CLI 0.4.0; the canonical CLI README remains linked throughout.

Brand colors, the static symbol, and licensed font assets come from `packages/brand`. Manrope is used for headings and identity; reading text and controls use the shared Inter/system stack. Website reading sizes and layout remain local. See `public/brand.css` for the consumer adapter. Fonts are self-hosted under the included SIL Open Font License. No external scripts, trackers, runtime libraries, or font requests are required.
...
The follow-up copy and interaction review is documented in `REVIEW.md`, including product descriptions corrected, validation, and source discrepancies outside the website edit scope.
