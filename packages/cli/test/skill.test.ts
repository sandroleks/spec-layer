...
describe('buildSkillGuide', () => {
   ...
   * `annotateGroups`) under `$extensions["com.spec-layer"].generated_description`
   * on a token group, a key whose name states its own origin. A rewording
   * that names only a COMPONENT's `guidelines` block, or that pins the
   * extension case to `tokens/` specifically, is still false: `bundle.json`
   * (files.ts ~line 167, the raw published bundle) also carries a
   * foundation's own `guidelines` block and, via `foundation.ai` (the
   * inlined `foundationDtcgDocument`), the same extension key outside
   * `tokens/` entirely. So the guide must name both content shapes without
   * tying the token-group one to a single directory. Kept consistent with
   * the same claim in the plugin's downloadable skill
  ...
  it('names the real provenance split instead of claiming no model wrote anything', () => {
    ...
    // foundation's own), and pinning the extension case to tokens/ alone
    // (bundle.json carries the same key too, outside tokens/).
