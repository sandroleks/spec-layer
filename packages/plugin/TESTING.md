...
1. Open **Foundations** and wait for local variable collections and
...
7. **Hidden elements, off.** On a component with two boolean-controlled layers
   ...
   **Document hidden elements** unchecked. The frames match a build of the
   same component from this build with the option never turned on, and the
   Library badge reads in sync.
...
18. **Rebuild from 5.1.0.** Open a file with a doc generated by 5.1.0. The
    Library row reads "Rebuild needed" and the banner "Some docs are from an
    older plugin version" appears. Rebuild it: Overview, Variants guide, Anatomy roles, Do and Don't,
...
27. **Rebuild with AI writing on.** Open a Library that holds a document
    ...
    Expected: the button's tooltip reads "In docs made with AI writing, AI
    rewrites Keyboard and fills empty sections, if AI writing is on.
    Everything else is kept."; after the rebuild, Overview, Semantics and the DO and DON'T cards
...
38. **Filling a placeholder.** Type over the Pointer and touch guidance and
    ...
    A known limit: a Keyboard row filled on one side only, or a Do or Don't
    card whose rule is still guidance, is dropped on Update and its guidance
    comes back, so what was typed into it is lost.
39. **Untouched placeholders never leak.** On the same doc, Copy for AI and
    check the brief carries no guidance text. Its `guidelines` block reads
    `origin: authored`, because a person wrote everything in it. With
    Settings on Markdown, the Interactions section reads "Written on the
    Figma canvas, not generated by AI." instead of the AI line.
...
12. **Show changes** lists what changed. Generate a component doc
    ...
    Library. Each row reads **Update available**. Open **Show
...
13. A doc whose stored baseline was dropped shows "Source changed" with
    "Update this doc once to enable change lists." and offers no list. To
    produce one, document a component large enough that the console logs
    `[Spec Layer] baseline dropped`, change its source, and refresh the
    Library. Run
...
- [ ] Publish (Pro license, file with foundation + 2 component docs): the footer
      ...
      the primary reads "Publishing…"; a toast reads "Published 1.0.0. Anyone
      with the pull key can pull this version." (an update to an existing
      library reads "Published <version>. Developers get this version on
      their next pull."); the screen then shows the **Published**
...
- [ ] Markdown setup: with Markdown chosen in Settings > Export, both setup
      ...
      **Copy** puts exactly that on the clipboard. With YAML chosen the line
      carries no flag. Run the Markdown command in an empty directory; it
      needs `spec-layer` 0.10.0 or later, and `npm view spec-layer version`
      shows the `latest` npm serves. `speclayer.json` carries
...
- [ ] First version: a file with a Foundation doc and two component docs,
      never published. Open Publish; the Version block reads "The first
      publish creates this version." with First version prefilled 1.0.0. Type `2.0` and see "Use three
...
- [ ] Pre-versioning library: a library whose last publish predates library
      versions on the proxy (2026-09-16). Open
      Publish; the block says the library has no version yet and the next
      publish creates 1.0.0. Publish; pills appear at v1.0.0.
- [ ] Deploy order: before any plugin release, confirm the deployed proxy
