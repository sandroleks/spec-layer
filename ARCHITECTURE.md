...
Hashed implies rendered: `FoundationTextMetrics` carries only the four values a frame draws, and the rendered text row carries no `boundVariables`, because a hash that moved on letter spacing or a rebound colour would offer an Update that produced a byte-identical frame. Extraction stays complete — `FoundationTextStyle` keeps every field — so rendering more later is a matter of moving fields back into the projection. Ids and `extractedAt` are excluded because they never appear in that output at all.
